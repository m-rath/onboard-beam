...
def run():
    ...
    with beam.Pipeline(options=pipeline_options) as p:
        ...
        # 1) Read TableRow records from BigQuery table source.
        ...
        # 3) Convert schema to string (for input to schema_fn)...
        schema_str = (
            field_schemas 
            | beam.Distinct() 
        ...
        # ...then to TableSchema (for output of schema_fn)...  (and eventually singleton)
        # table_schema = schema_str | beam.Map(beam.io.gcp.bigquery_tools.get_table_schema_from_string)

        # 4) Define schema function for dynamic staging of WriteToBigQuery sink
        def schema_fn(element, schema_str):
            return beam.io.gcp.bigquery_tools.get_table_schema_from_string(schema_str)

        # 5) Format records to be sparse, with more nulls.
        ...
        records | 'Write' >> WriteToBigQuery(
            ...
            schema = schema_fn, # would a lambda function work?  schema = lambda row: schema_str; then no side_inputs
            schema_side_inputs = (AsSingleton(schema_str), ), #A tuple with AsSideInput PCollections to be passed to the schema callable (if one is provided)
